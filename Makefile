...
PG_URL=postgres://postgres:postgres@localhost:5432/auth_microservice

run: ### run app
	go run cmd/authorization/main.go -db_url '$(PG_URL)' -port localhost:50051 -log_level debug -cert ./tls/cert.pem -key ./tls/key.pem -token_ttl 1000 -jwt_secret secret
